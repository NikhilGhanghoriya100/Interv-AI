...
export const generateQuestion = async (req, res) => {
  try {
    ...
    if (!userPrompt.trim()) {
      return res.status(400).json({
        message: "Prompt cotent is empty.",
      });
    }
...
export const submitAnswer = async (req,res) =>{
    ...
    try {
      ...
      console.error("Failed to parse AI response JSON:", aiResponse);
    }
    ...
    question.feedback = parsed.feedback || "Answer recorded successfully.";
...
export const getUserHistory = async (req, res) => {
  try {
    ...
    const averageScore = totalInterviews ? Number((totalScore / totalInterviews).toFixed(1)) : 0;
    const highestScore = totalInterviews ? Math.max(...interviews.map((i) => i.finalScore || 0)) : 0;

    const formattedList = interviews.map((item) => {
      const qCount = item.questions?.length || 0;
      let conf = 0, comm = 0, corr = 0;
      if (item.questions && qCount > 0) {
        item.questions.forEach((q) => {
          conf += q.confidence || 0;
          comm += q.communication || 0;
          corr += q.correctness || 0;
        });
        conf = Number((conf / qCount).toFixed(1));
        comm = Number((comm / qCount).toFixed(1));
        corr = Number((corr / qCount).toFixed(1));
      }
    ...
    return res.status(200).json({
      ...
      interviews: formattedList,
    });
  } catch (error) {
    return res.status(500).json({ message: `Failed to fetch history: ${error.message || error}` });
  }
};
...
export const getInterviewById = async (req, res) => {
  try {
    ...
    return res.status(200).json({
      ...
      finalScore: Number(interview.finalScore || finalScore.toFixed(1)),
      confidence: Number(avgConfidence.toFixed(1)),
      communication: Number(avgCommunication.toFixed(1)),
      correctness: Number(avgCorrectness.toFixed(1)),
      questionWiseScore: interview.questions.map((q) => ({
    ...
    return res.status(500).json({ message: `Failed to fetch interview: ${error.message || error}` });
  }
};
...
export const deleteInterview = async (req, res) => {
  try {
    ...
    return res.status(500).json({ message: `Failed to delete interview: ${error.message || error}` });
  }
};